...
Multi-Tab Workspace &middot; AI Co-Pilot &middot; Live Market Intelligence &middot; Cross-Exchange Arbitrage &middot; Proactive Squeeze Radar
...
NexusDesk is a desktop trading command center built with Electron. It gives you a unified workspace with live charts, a multi-tab coin workspace, an AI trading assistant, an arbitrage scanner, a proactive market intelligence dashboard, and a portfolio manager -- all in one dark-mode, premium interface.
...
- **Multi-Tab Workspace System** -- Monitor multiple coins simultaneously by switching between tabs in the top control bar. Supports opening (`+`), closing (`x`), double-click inline tab renaming, and full cross-pane synchronization across TradingView, Coinglass, and CoinMarketCap without losing context.
- **Automatic Workspace Persistence** -- Open tabs, active tab selection, pane split ratios, and last active symbols save automatically to `%AppData%\NexusDesk\hub-settings.json` and restore seamlessly across app restarts.
...
- **Live Funding Rate Sparklines** -- Track 12-hour derivative funding trends visually within the watchlist, using dynamic micro-charts rendered via Chart.js and CCXT.
...
| Source code on GitHub | Public repo | Zero secrets in the code |
...
### 1. Install and Run for Development
...
Once finished, look inside the `dist/` folder. You will find `NexusDesk Setup 1.0.0.exe` ready to use.
...
NexusDesk/
+-- electron/
|   +-- main.cjs              Main process -- IPC, security, trade engine, tab state
...
|   +-- control.html          Top control bar with Multi-Tab Workspace
|   +-- preload.cjs           Electron preload (tab IPC bindings)
|   +-- splitter.html         Layout drag handle
+-- docs/
|   +-- ARCHITECTURE.md       Technical system architecture & IPC specs
|   +-- dashboard.png         Dashboard screenshot
+-- multi-tab-workspace.md    Multi-tab roadmap & design specs
+-- .gitignore
+-- LICENSE
+-- README.md
+-- llms.txt
...
| Version | Date | Change |
|---|---|---|
| 1.4.0 | 2026-07-25 | Feature: Multi-Tab Workspace System with cross-pane sync, inline rename & JSON persistence |
